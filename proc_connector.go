...
// New creates a new process Walker.
func New(getInitialState bool) (pc *ProcConnector, err error) {
	pc = &ProcConnector{
		running:    false,
		activePids: map[int]Process{},
	}
	...
	if getInitialState {
		...
		dirEntries, err := proc.Readdirnames(-1)
		if err != nil {
...
func GetCmdline(pid int) (name, cmdline string) {
	...
	if name == "" {
		if commBuf, err := ioutil.ReadFile(path.Join("/proc", strconv.Itoa(pid), "comm")); err != nil {
...
// IsRunning tells whether the proc connector is really working.
